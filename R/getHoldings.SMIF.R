...
#'
#' @param serverPW Character; string of the SQL server's password
...
#' @examples
#' \donttest{
#' getHoldings.SMIF(FALSE, ipkey="The IP Key", serverPW="The Server Password")
#' }
...
"getHoldings.SMIF" <- function(auto.assign=TRUE,
  ...
  if(!is.environment(env) | !is.character(varName)){ auto.assign = FALSE }
  # browser()
  if(auto.assign){
    assign(x = varName, value = positions, envir = env)
    return(varName)
  }else{
    return(positions)
  }
